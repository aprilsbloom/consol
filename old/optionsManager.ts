...
import { hexToAnsi, REGEX } from '../src/utils';
import type { Format, LogType, LoggerOptions, Style } from '../src/types';
...
export class OptionsManager {
	...
	protected formatDateTemplate(msg: string): string {
		return msg.replaceAll(REGEX.DATE, (_, date) => strftime(date));
	}

	protected formatLevelTemplateAnsi(msg: string, level: LogLevel): string {
		return msg.replaceAll('!{level}', this.options.format.level[LogLevel[level].toLowerCase() as LogType].ansi!);
	}

	protected formatLevelTemplateStr(msg: string, level: LogLevel): string {
		return msg.replaceAll('!{level}', this.options.format.level[LogLevel[level].toLowerCase() as LogType].str);
	}

	protected formatMessageTemplate(msg: string): string {
		return msg.replaceAll('!{message}', msg);
	}

	protected formatStylesTemplate(msg: string): string {
		return msg.replaceAll(REGEX.STYLES, (full, style: Style) => {
	...
	protected formatHexTemplate(msg: string): string {
		return msg.replaceAll(REGEX.HEX, (_, type, hex) => {
